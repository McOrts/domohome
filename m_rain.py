...
logging.basicConfig(filename='/home/pi/rain_event_error.log',
  level=logging.DEBUG,
...
#    print rain
    con = mdb.connect('localhost',
                      ...
                      'measurements');
    cursor = con.cursor()
    sql = "INSERT INTO rain(precipitation) VALUES ('%s')" % (rain)
    cursor.execute(sql)
    sql = []
    con.commit()
    con.close()
...
# Function for send an email notifing the door is open 
def send_email_open_door(precipitation):
  # Specifying the from and to addresses
  toaddr = config['mail']['toaddr']
  #cc = config['mail']['cc']
  #bcc = ['chairman@slayerscouncil.uk']
  fromaddr = 'mcortshome@gmail.com'
  message_subject = "[RAIN] Home rain alert"
  message_text = "It's raining at home %s mm/10min (%s)" % (str(precipitation),time.strftime("%c"))
  message = "From: %s\r\n" % fromaddr + "To: %s\r\n" % toaddr + "Subject: %s\r\n" % message_subject + "\r\n"  + message_text
  toaddrs = [toaddr]
  # Sending the mail  
  try:
...
def calculate_precipitation(factor):
    global count
    mm_per_period = count * factor
    mm_per_period = round(mm_per_period,2)
    return mm_per_period 
...
def send_data_IOT_cloud(p_rain):
  try:
    ...
    artikcloud.configuration.access_token = config['domohome_rain']['device_token']
...
def publish_MQTT_messages (p_event):
  try:
    client = mqtt.Client()
    # compose the message 
    topic = config['domohome_rain']['topic']
...
GPIO.setup(config['domohome_rain']['gpio_pin'], GPIO.IN, GPIO.PUD_UP)
GPIO.add_event_detect(config['domohome_rain']['gpio_pin'], GPIO.FALLING, callback=spin)

while True:
    count = 0
    time.sleep(interval)
    if count>0:
      rain = calculate_precipitation(rain_factor)
      print (rain)
      insertDB (rain)
      publish_MQTT_messages(rain)
      send_data_IOT_cloud(rain)
      if rain > config['domohome_rain']['max_warning']:
      	send_email_open_door(rain)
